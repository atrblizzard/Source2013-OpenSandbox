...
ChunkFileResult_t CChunkFile::Open(const char *pszFileName, ChunkFileOpenMode_t eMode)
{
	...
	else if (eMode == ChunkFile_Write)
	{
		...
		m_hFile = g_pFullFileSystem->Open(pszFileName, "wb", "DEFAULT_WRITE_PATH");
...
ChunkFileResult_t CChunkFile::WriteLine(const char *pszLine)
{
	if (pszLine != NULL)
	{
		...
		if (m_nCurrentDepth > 0)
		{
			...
			int nWritten = g_pFullFileSystem->Write(m_szIndent, m_nCurrentDepth, m_hFile);
		...
		int nWritten = g_pFullFileSystem->Write(pszLine, nLen, m_hFile);
		if (nWritten != nLen)
		{
			return(ChunkFile_Fail);
		}
